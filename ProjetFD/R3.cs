...
namespace ProjetFD
{
    public partial class R3 : Form
    {
        ...
        {
            SqlDataAdapter sdf = new SqlDataAdapter("select * from M3 where TodaySDate between '" + StartDate.Value.ToString("yyy-MM-dd") + "' and '" + dtEnd.Value.ToString("yyy-MM-dd") + "'", conn);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            cmd = new SqlCommand(" select * from M3 where  TodaySDate between '" + date1 + "' and '" + date2 + "'", conn);
            dr = new SqlDataAdapter(cmd);
            dr.Fill(dt);

            CrystalReport17 cr = new CrystalReport17();
            cr.Database.Tables["M3"].SetDataSource(dt);
