...
namespace ProjetFD
{
    public partial class R11 : Form
    {
        ...
        {
            SqlDataAdapter sdf = new SqlDataAdapter("select * from M11 where TodaySDate between '" + StartDate.Value.ToString("yyy-MM-dd") + "' and '" + dtEnd.Value.ToString("yyy-MM-dd") + "'", conn);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            cmd = new SqlCommand(" select * from M11 where  TodaySDate between '" + date1 + "' and '" + date2 + "'", conn);
            dr = new SqlDataAdapter(cmd);
            dr.Fill(dt);

            CrystalReport25 cr = new CrystalReport25();
            cr.Database.Tables["M11"].SetDataSource(dt);
