...
namespace ProjetFD
{
    public partial class R8 : Form
    {
        ...
        {
            SqlDataAdapter sdf = new SqlDataAdapter("select * from M8 where TodaySDate between '" + StartDate.Value.ToString("yyy-MM-dd") + "' and '" + dtEnd.Value.ToString("yyy-MM-dd") + "'", conn);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            cmd = new SqlCommand(" select * from M8 where  TodaySDate between '" + date1 + "' and '" + date2 + "'", conn);
            dr = new SqlDataAdapter(cmd);
            dr.Fill(dt);

            CrystalReport22 cr = new CrystalReport22();
            cr.Database.Tables["M8"].SetDataSource(dt);
