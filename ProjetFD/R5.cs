...
namespace ProjetFD
{
    public partial class R5 : Form
    {
        ...
        {
            SqlDataAdapter sdf = new SqlDataAdapter("select * from M5 where TodaySDate between '" + StartDate.Value.ToString("yyy-MM-dd") + "' and '" + dtEnd.Value.ToString("yyy-MM-dd") + "'", conn);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            cmd = new SqlCommand(" select * from M5 where  TodaySDate between '" + date1 + "' and '" + date2 + "'", conn);
            dr = new SqlDataAdapter(cmd);
            dr.Fill(dt);

            CrystalReport19 cr = new CrystalReport19();
            cr.Database.Tables["M5"].SetDataSource(dt);
