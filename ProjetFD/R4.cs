...
namespace ProjetFD
{
    public partial class R4 : Form
    {
        ...
        {
            SqlDataAdapter sdf = new SqlDataAdapter("select * from M4 where TodaySDate between '" + StartDate.Value.ToString("yyy-MM-dd") + "' and '" + dtEnd.Value.ToString("yyy-MM-dd") + "'", conn);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            cmd = new SqlCommand(" select * from M4 where  TodaySDate between '" + date1 + "' and '" + date2 + "'", conn);
            dr = new SqlDataAdapter(cmd);
            dr.Fill(dt);

            CrystalReport18 cr = new CrystalReport18();
            cr.Database.Tables["M4"].SetDataSource(dt);
