...
namespace ProjetFD
{
    public partial class R2 : Form
    {
        ...
        {
            SqlDataAdapter sdf = new SqlDataAdapter("select * from M2 where TodaySDate between '" + StartDate.Value.ToString("yyy-MM-dd") + "' and '" + dtEnd.Value.ToString("yyy-MM-dd") + "'", conn);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            cmd = new SqlCommand(" select * from M2 where  TodaySDate between '" + date1 + "' and '" + date2 + "'", conn);
            dr = new SqlDataAdapter(cmd);
            dr.Fill(dt);

            CrystalReport16 cr = new CrystalReport16();
            cr.Database.Tables["M2"].SetDataSource(dt);
